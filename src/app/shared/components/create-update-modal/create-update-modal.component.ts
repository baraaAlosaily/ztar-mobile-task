...
export class CreateUpdateModalComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.dataBase === 'books') {
      if (
        ...
        changes['selectedItem']?.currentValue !== null
      ) {
  ...
  onSubmit() {
    if (this.dataBase === 'books') {
      if (this.booksForm.valid) {
        this.addUpdateEmitter.emit({
          data: this.booksForm.value,
          action: this.button,
        });
      ...
      if (this.categoriesForm.valid) {
        this.addUpdateEmitter.emit({
          data: this.categoriesForm.value,
          action: this.button,
        });
        this.closeModal.nativeElement.click();
        this.categoriesForm.reset();
