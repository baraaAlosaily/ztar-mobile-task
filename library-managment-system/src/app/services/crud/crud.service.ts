...
export class CrudService {
  ...
  // getItems
  getItems(pageSize: number, pageNumber: number ,dbName:string ,orderBy:string , searchTerm: string = ''): Observable<any[]> {
    return new Observable((observer) => {
      let reference = ref(this.db, dbName);
      let referenceQuery;

      if (searchTerm) {
        const searchTermEnd = searchTerm + '\uf8ff';
        referenceQuery = query(reference, orderByChild(orderBy), startAt(searchTerm), endAt(searchTermEnd));
      } else {
        referenceQuery = query(reference, orderByChild(orderBy));
      }

      const startIndex = (pageNumber - 1) * pageSize;
      referenceQuery = query(referenceQuery, limitToFirst(startIndex + pageSize));

      onValue(referenceQuery, (snapshot) => {
  ...
  getAllItems(dbName: string, orderBy: string): Observable<any[]> {
    return new Observable((observer) => {
      let reference = ref(this.db, dbName);
      let referenceQuery = query(reference, orderByChild(orderBy));
  ...
  // updateItemById
  updateItemById(id: string, updatedData: any,dbName:string): Observable<void> {
    return new Observable((observer) => {
      const reference = ref(this.db, `${dbName}/${id}`);
  ...
  // getItemsSize
  getItemsSize(dbName:string,orderBy:string,searchTerm:string=''): Observable<number> {
    return new Observable((observer) => {
      let reference = ref(this.db, dbName);
      let referenceQuery;
      if (searchTerm) {
      ...
      get(referenceQuery).then((snapshot) => {
        let size = 0;
        snapshot.forEach(() => {
          size++;
        });
